...
(async ()=>{
    ...
    const FormToKeyValuePair = (target)=>{
        if (!target) {return '';}
        let param = '';
        if (typeof target === 'string') {try {target = JSON.parse(target);
            for (const [key, value] of Object.entries(target)) {param += (key+'='+value+'&');}} catch (error) {return '';}
        } else if (TypeOf(target) === 'Object') {
            for (const [key, value] of Object.entries(target)) {param += (key+'='+value+'&');}
        } else if (TypeOf(target) === 'Map') {
            for (const [key, value] of target) {param += (key+'='+value+'&'); }
        } else if (typeof target === 'number') {return target;} else { return '';}
        return param.slice(0, -1);
    };
    ...
    const PostFormRequestJSON = (url, param, resolveCallBack, rejectCallBack)=>{
        if (!url || !param ) {return 'Error';}
        if (!resolveCallBack) {resolveCallBack = (data)=>{console.log(data);}}
        if (!rejectCallBack) {rejectCallBack = ()=>{console.log('Error');}}
        if (typeof param !== 'string') {param = FormToKeyValuePair(param);}
        fetch(url, {
            method: 'POST',
            headers: {
